...
void printList(Node* head)
{
    ...
    while (temp != NULL)
    {
        printf("%-7s %s\n", (temp->data.status == 1) ? "okay" : "error", temp->data.url);
...
void bubbleSort(Node** head)
{
    ...
    do
    {
        ...
        {
            if (strcmp(ptr->data.url,ptr->next->data.url) > 1)
            {
...
void checkDups(Node** head)
{
    ...
    while (current != NULL)
    {
        ...
        while (runner != NULL)
        {
            Node* next = runner->next;  // Save next since runner may be deleted
            if (strcmp(current->data.url, runner->data.url) == 0)
            {
                runner = deleteNode(head, runner);
            }
            else
            {
                runner = next;
