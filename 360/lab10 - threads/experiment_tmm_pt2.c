...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (clock_gettime(CLOCK_REALTIME, &realTimeStart) == -1)
    {
    ...
    if (clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &cpuStart) == -1)
    {
        perror("clock_gettime");
        return 1;
    }
    ...
    */
    ...
    matMulThreadStats = (MatMulThreadStats*)malloc(sizeof(MatMulThreadStats) * (nThreads > 0 ? nThreads : 1));
    ...
    */
    ...
    if (clock_gettime(CLOCK_REALTIME, &realTimeEnd) == -1)
    {
        perror("clock_gettime");
        return 1;
    }
    ...
    if (clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &cpuEnd) == -1)
    {
        perror("clock_gettime");
        return 1;
    }
    ...
    */
    ...
    {
        cpuTimeTotal = 0.0;
        cpuTimeTotal += matMulThreadStats[0].cpuTime;
        //printf("Rows multiplied: %d\n",matMulThreadStats[i].nRowsDone);
    ...
    */
    ...
    printf("cpu time: %f\n", tspecDiff(cpuStart, cpuEnd));
    printf("wall clock time: %f\n",tspecDiff(realTimeStart, realTimeEnd));
